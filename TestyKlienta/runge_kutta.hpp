...
namespace Physics
{
	...
	public:
		...
		static void RK4(float oldTime, float deltaTime,const std::array<float, stateLength>& oldState,
			const RungeKuttaRightHandSide<stateLength>& rightHandSide, std::array<float, stateLength>& newState);
		~RungeKutta() = delete;
	};

	template <std::size_t stateLength>
	void RungeKutta<stateLength>::RK4(float oldTime, float timeStep,const std::array<float, stateLength>& oldState
		,const RungeKuttaRightHandSide<stateLength>& rightHandSide, std::array<float, stateLength>& newState)
	{
